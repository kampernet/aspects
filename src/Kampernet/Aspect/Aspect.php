...
class Aspect {
	 ...
	 *
	 * @param array $advice an array of Aspect objects
	 * @return Aspect
	 */
...
//			$this->object->setOriginalClassName($this->className);
	...
	private function applyBeforeAdvice(ReflectionClass $class, ReflectionMethod $method, ReflectionClass $adviceClass) {
		...
		foreach ($methods as $advice) {
			$name = $advice->getName();
			$info = self::getAnnotations($advice);
			// preg match
			...
			// exact match
			$test = Aspect::POINTCUT_BEFORE . $this->className . $method->getName();
			if ($name == $test || $info['match'] == $test) {
				$adviceMethods [] = $advice;
			}
		}
	 ...
	 * @return Aspect
	 */
	public static function getAspectedInstance($classname) {

		// get the aspects this object has
		$c = new ReflectionClass($classname);
		$info = self::getAnnotations($c);
		$aspects = null;
